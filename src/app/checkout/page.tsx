    ...
    return (
      <div className="flex flex-col min-h-screen bg-brand-surfaceDim">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
          <h2 className="text-xl font-bold text-brand-dark">سبد خرید شما خالی است</h2>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-brand-surfaceDim">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <h1 className="text-2xl font-black text-brand-dark mb-8 flex items-center gap-2">
        ...
        <form onSubmit={handleCompleteOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="bg-white border border-brand-border rounded-2xl p-6 sm:p-7 shadow-card space-y-5">
              <h2 className="text-sm font-bold text-brand-dark pb-3 border-b border-brand-border flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-brand-surfaceDim border border-brand-border focus:border-brand-primary rounded-xl py-2.5 px-3 outline-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-brand-surfaceDim border border-brand-border focus:border-brand-primary rounded-xl py-2.5 pr-9 pl-3 outline-none dir-ltr text-left font-mono"
                    />
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-brand-surfaceDim border border-brand-border focus:border-brand-primary rounded-xl py-2.5 pr-9 pl-3 text-xs outline-none dir-ltr text-left font-mono"
                  />
            ...
            <div className="bg-white border border-brand-border rounded-2xl p-6 sm:p-7 shadow-card space-y-4">
              <h2 className="text-sm font-bold text-brand-dark pb-3 border-b border-brand-border flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                <label
                  ...
                  className={`p-4 rounded-xl border cursor-pointer transition-all flex flex-col justify-between ${
                    gateway === "zarinpal"
                      ? "border-brand-primary bg-teal-50/50 shadow-xs"
                      : "border-brand-border hover:bg-neutral-50"
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-bold text-brand-dark">زرین‌پال</span>
                  ...
                  <span className="text-[11px] text-brand-muted">کلیه کارت‌های عضو شتاب</span>
                </label>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-bold text-brand-dark">نکست‌پی</span>
                    <span className="w-3.5 h-3.5 rounded-full border border-brand-primary flex items-center justify-center">
                      {gateway === "nextpay" && <span className="w-2 h-2 rounded-full bg-brand-primary" />}
                    </span>
                  </div>
                  <span className="text-[11px] text-brand-muted">درگاه پرداخت سریع شاپرک</span>
                </label>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-bold text-brand-dark">تتر (USDT)</span>
                    <span className="w-3.5 h-3.5 rounded-full border border-brand-primary flex items-center justify-center">
                      {gateway === "crypto" && <span className="w-2 h-2 rounded-full bg-brand-primary" />}
                    </span>
                  </div>
                  <span className="text-[11px] text-brand-muted">تسویه با رمزارز TRC20</span>
                </label>
              </div>
          ...
          <div className="lg:col-span-5 bg-white border border-brand-border rounded-2xl p-6 shadow-card space-y-5 sticky top-28">
            <h3 className="font-bold text-sm text-brand-dark pb-3 border-b border-brand-border">
              اقلام سفارش ({cart.length})
            </h3>

            <div className="divide-y divide-brand-border max-h-60 overflow-y-auto pr-1">
              {cart.map((item) => {
                const p = calculateProductPrice(item.product);
                const itemTotal = p.isPerThousand
                  ? Math.round((p.toman * item.quantity) / 1000)
                ...
                return (
                  <div key={item.product.id} className="py-2.5 flex justify-between items-center text-xs">
                    <div>
                      <span className="font-bold text-brand-dark block line-clamp-1">{item.product.customTitle}</span>
                      <span className="text-[10px] text-brand-muted">تعداد: {item.quantity}</span>
                    </div>
                    <span className="font-bold text-brand-dark font-mono shrink-0">
                      {new Intl.NumberFormat("fa-IR").format(itemTotal)} ت
                    </span>
                  </div>
                );
              })}
            ...
            <div className="pt-3 border-t border-brand-border space-y-2 text-xs">
              ...
              <div className="pt-2 border-t border-brand-border flex justify-between items-baseline">
                <span className="font-bold text-sm text-brand-dark">مبلغ نهایی:</span>
                <div className="text-left">
                  <span className="text-2xl font-black text-brand-dark font-mono">
            ...
            <div className="flex items-center justify-center gap-1.5 text-[11px] text-teal-800 bg-teal-50 p-2.5 rounded-xl">

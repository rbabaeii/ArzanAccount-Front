...
export default function AdminTagsPage() {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Header Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-brand-border dark:border-slate-800 shadow-sm">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-lg font-black text-slate-900 dark:text-white">
                مدیریت برچسب‌ها و تگ‌های هوشمند (Smart Tags)
              </h1>
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-teal-200 dark:border-teal-800 bg-teal-50/70 dark:bg-teal-950/40 text-brand-primary dark:text-teal-300 hover:bg-teal-100 text-xs font-bold transition-all disabled:opacity-50"
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-brand-primary hover:bg-teal-700 text-white text-xs font-bold shadow-md shadow-teal-900/10 transition-all"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-brand-border dark:border-slate-800 shadow-sm space-y-1">
          <div className="flex items-center justify-between text-slate-400">
            <span className="text-xs font-medium">کل تگ‌های یکتا</span>
            <Hash className="w-4 h-4 text-brand-primary dark:text-teal-400" />
          </div>
          <div className="text-2xl font-black text-slate-900 dark:text-white font-mono">
            {formatNumber(totalTagsCount)}
          </div>
          <p className="text-[11px] text-slate-500">برچسب فعال در دیتابیس</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-brand-border dark:border-slate-800 shadow-sm space-y-1">
          ...
          <p className="text-[11px] text-slate-500">از کل {formatNumber(products.length)} کالا</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-brand-border dark:border-slate-800 shadow-sm space-y-1">
          ...
          <p className="text-[11px] text-slate-500">نیازمند برچسب‌گذاری</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-brand-border dark:border-slate-800 shadow-sm space-y-1">
          ...
          <p className="text-[11px] text-slate-500">با بیشترین اشتراک کالاها</p>
        </div>
      </div>

      {/* Preset Tag Clusters */}
      <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-brand-border dark:border-slate-800 shadow-sm space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-800 dark:text-slate-200">
            <Layers className="w-4 h-4 text-brand-primary dark:text-teal-400" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5">
          {PRESET_CLUSTERS.map((cluster, idx) => (
            <div
              ...
              className="p-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40 hover:border-brand-primary dark:hover:border-teal-400 cursor-pointer transition-all space-y-2 group"
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-brand-primary dark:text-teal-400 opacity-0 group-hover:opacity-100 font-bold transition-opacity">
                  + افزودن
                </span>
              </div>
              <div className="flex flex-wrap gap-1">
                {cluster.tags.slice(0, 3).map((t, i) => (
                  <span
                    key={i}
                    className="text-[10px] px-1.5 py-0.5 rounded bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300"
                  >
                    #{t}
                  </span>
                ))}
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-brand-border dark:border-slate-800 shadow-sm">
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto text-xs">
          ...
          <select
            ...
            className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-slate-200 outline-none"
          >
      ...
      {/* Tags Grid */}
      {isLoading ? (
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-16 text-center border border-brand-border dark:border-slate-800 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredTags.map((item, idx) => (
            <div
              key={idx}
              className="bg-white dark:bg-slate-900 border border-brand-border dark:border-slate-800 rounded-3xl p-5 shadow-sm hover:border-brand-primary/50 dark:hover:border-teal-500/50 transition-all flex flex-col justify-between gap-4"
            >
              <div>
                <div className="flex items-start justify-between gap-2 pb-3 border-b border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-2">
                    <span className="w-8 h-8 rounded-xl bg-teal-50 dark:bg-teal-950/60 text-brand-primary dark:text-teal-400 flex items-center justify-center font-bold text-xs">
                      #
                    </span>
                    <span className="font-black text-sm text-slate-900 dark:text-white break-all">
                      {item.name}
                    </span>
                  </div>

                  <span className="px-2.5 py-1 rounded-full text-[11px] font-bold bg-teal-50 dark:bg-teal-950/70 text-teal-800 dark:text-teal-300 border border-teal-200 dark:border-teal-800/80 shrink-0 font-mono">
                ...
                <div className="mt-3 space-y-1.5">
                  ...
                  {item.sampleProducts && item.sampleProducts.length > 0 ? (
                    <div className="space-y-1">
                      ...
                      {item.count > 3 && (
                        <span className="text-[10px] text-brand-primary dark:text-teal-400 font-semibold block pt-0.5">
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-brand-primary text-slate-700 dark:text-slate-300 hover:text-brand-primary dark:hover:text-teal-400 text-xs font-bold transition-colors"
                >
